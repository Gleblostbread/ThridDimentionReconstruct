...
def align_via_procrustes_anchors(source: NDArray, target: NDArray, anchor_indices: list | NDArray):
    """
    Вычисляет преобразование подобия по якорным точкам,
    ...
    # Центрирование по якорям
    cs = src_anchors.mean(axis=0)
    ct = tgt_anchors.mean(axis=0)
    s_c = src_anchors - cs
    t_c = tgt_anchors - ct
    
    # Масштаб (вычисляется только по якорям)
    ...
    # Кросс-ковариация и SVD (только по якорям)
    H = s_n.T @ t_n
    ...
    # Применяем оба варианта ко всему облаку
    aligned_a = ((source - cs) / scale_s @ R_a) * scale_t + ct
    aligned_b = ((source - cs) / scale_s @ R_b) * scale_t + ct
    
    # 5. Выбираем вариант с минимальной ошибкой на якорях
